...
public class QuestionSendActivity extends AppCompatActivity implements View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    private static final int PERMISSIONS_REQUEST_CODE = 100;  // 許可の結果を受け取る際に識別するための数値
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == CHOOSER_REQUEST_CODE) {
            ...
            // 画像を取得
            //dataがnullかdata.getData()の場合はカメラで撮影したときなので画像の取得にmPictureUriを使
            ...
            try {
                ...
                InputStream inputStream = contentResolver.openInputStream(uri);  // コンテンツURIに関連付けられたコンテンツへのストリームを開く
                image = BitmapFactory.decodeStream(inputStream); // 入力ストリームをビットマップにデコード
                inputStream.close();
            } catch (Exception e) {
                return;
            }
            ...
            // createBitmap:オプションの行列で変換された、ソースビットマップのサブセットから不変ビットマップを返す。
        ...
        if (v == mImageView) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    // 第1引数には許可を求めたいパーミッションを、第2引数は許可の結果を受け取る際に識別するための数値を
    ...
    private void showChooser() {
        ...
        // Category:Actionの振る舞いを明確にするためのもの  CATEGORY_OPENABLE:必ずURIが受け渡されること
        ...
        // EXTRA_INITIAL_INTENTSを使って、Chooser画面にギャラリー選択とカメラ撮影の２つを入れることができる
    ...
    public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
        mProgress.dismiss();
        // 必ずダイアログのshowメソッドを呼び出した後にdismissメソッドを呼び出す
