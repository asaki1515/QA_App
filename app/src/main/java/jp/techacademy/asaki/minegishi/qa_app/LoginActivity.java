...
public class LoginActivity extends AppCompatActivity {
    ...
    ProgressDialog mProgress;  // アカウント作成中プログレスダイアログを表示

    FirebaseAuth mAuth;
    // 処理の完了を受け取るリスナー
    OnCompleteListener<AuthResult> mCreateAccountListener; // アカウント作成処理
    OnCompleteListener<AuthResult> mLoginListener;  // ログイン処理用
        ...
        mLoginListener = new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(Task<AuthResult> task) {
                ...
                if (task.isSuccessful()) {
                    ...
                    // ConstクラスのUsersPATHの名前の子の場所の中の子であるuserのUidを取得
                    ...
                    if (mIsCreateAccount) {
                        ...
                        // Map:キーに関連付けてデータを格納するデータ構造
                        // Mapの生成:Map<キーの型, データの型> マップの名前 = new HashMap<キーの型, データの型>();
                        Map<String, String> data = new HashMap<String, String>();
                        data.put("name", name);
                        userRef.setValue(data);  // userのUidにMapのdataとして格納した名前を保存

                        // 表示名をPrefarenceに保存する
                        saveName(name);
                    } else {
                        // Firebaseから表示名を取得してPreferenceに保存
                        // addListenerForSingleValueEvent:Firebaseからデータを一度だけ取得する場合、
                        // DatabaseReferenceクラスが実装しているQueryクラスのaddListenerForSingleValueEventメソッドを使う
                        userRef.addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot snapshot) {
                                // DataSnapshot:効率的に生成された不変のデータコピー
                                Map data = (Map) snapshot.getValue();
                                saveName((String)data.get("name"));
                            }
                            @Override
                            public void onCancelled(DatabaseError firebaseError) {
                                //DatabaseError:操作が失敗したときにコールバックに渡される。
                                // 発生した特定のエラーの説明が含まれる。
                            }
                        });
                    }
